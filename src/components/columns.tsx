...
const DebouncedTextInput = React.memo(({ cell, table, handleSaveCell }) => {
  const [value, setValue] = useState(cell.getValue() ?? '');
  
  const debouncedHandleSaveCell = useDebouncedCallback(
    (newValue) => handleSaveCell(cell, newValue),
    300
  );
  ...
  const onChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValue(newValue);
    debouncedHandleSaveCell(newValue);
  }, [debouncedHandleSaveCell]);
...
const DebouncedNumberInput = React.memo(({ cell, table, handleSaveCell, precision = 0 }) => {
  const [value, setValue] = useState<number | undefined>(cell.getValue() ?? undefined);
  
  const debouncedHandleSaveCell = useDebouncedCallback(
    (newValue: number | undefined) => handleSaveCell(cell, newValue),
    300
  );

  const onChange = useCallback((newValue: number | undefined) => {
    setValue(newValue);
    debouncedHandleSaveCell(newValue);
  }, [debouncedHandleSaveCell]);
...
export const getColumns = (handleSaveCell: (cell: MRT_Cell<TableRow>, value: any) => void): MRT_ColumnDef<TableRow>[] => [
  ...
  },
  {
    ...
    header: 'QTY On-car',
    Edit: ({ cell, table }) => <DebouncedNumberInput cell={cell} table={table} handleSaveCell={handleSaveCell} />,
  },
  {
  ...
  },
  {
    ...
    header: 'Weight (lbs)',
    Edit: ({ cell, table }) => <DebouncedNumberInput cell={cell} table={table} handleSaveCell={handleSaveCell} precision={3} />,
  },
  {
